...
void testCUDA(std::vector<float>& output, const std::vector<glm::vec3>& coordinates, bool print_result)
{
   ...
   constexpr float search_radius = 2.0f;
   const std::vector<glm::vec3> queries = { { 4.0f, 3.0f, 1.0f } };

   std::vector<std::vector<int>> founds;
   auto start_time = std::chrono::steady_clock::now();
   kdtree.search( founds, glm::value_ptr( queries[0] ), static_cast<int>(queries.size()), search_radius );
   auto end_time = std::chrono::steady_clock::now();
   const auto search_time =
      static_cast<double>(std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time).count()) * 1e-9;
   for (size_t i = 0; i < founds.size(); ++i) {
      std::cout << "\n>> " << founds[i].size() << " nodes within " << search_radius << " units of ("
         << queries[i].x << ", " << queries[i].y << ", " << queries[i].z << ") in all dimensions, Search Time: "
         << search_time << "\n";
      std::cout << ">> List of k-d nodes within " << search_radius << "-unit search radius\n   ";
