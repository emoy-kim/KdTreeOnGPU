...
public:
   ...
   void getResult(std::vector<T>& output) const { if (Root != nullptr) getResult( output, Root.get(), 0 ); }
...
private:
   ...
   void getResult(std::vector<T>& output, KdtreeNode* node, int depth) const;
};
